...
class DB {
	...
	async deleteMenuItem(id) {
		...
		try {
			if (id) {
				await this.query(connection, `DELETE FROM menu WHERE id=?`, [
					id,
				]);
			}
		} finally {
	...
	async deleteUser(userId) {
		...
		try {
			if (userId) {
				await this.query(connection, `DELETE FROM user WHERE id=?`, [
					userId,
				]);
			}
		} finally {
	...
	async getID(connection, key, value, table) {
		const [
			rows,
		] = await connection.execute(`SELECT id FROM ${table} WHERE ${key}=?`, [
			value,
		]);
		if (rows.length > 0) {
			return rows[0].id;
